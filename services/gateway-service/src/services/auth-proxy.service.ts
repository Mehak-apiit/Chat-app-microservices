...
const resolveMessage = (status: number, data: unknown): string => {
    ...
    return status >= 500 ? 'Authentication service is currently unavailable. Please try again later.' : 'An error occurred while processing your request.';
};
const handleAxiosError = (error: unknown): never => {
    if(!axios.isAxiosError(error) || !error.response){
        throw new HttpError(500,'Authentication service is unavaliable');
    }
